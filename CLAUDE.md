...
- **moff** — Project governance in the role of Product Manager, sized for a solo
  developer with a sporadic designer. On first run conducts a setup interview that
  produces the governance charter: client, verifiable success
  criteria with named exclusions, communication contract (cadence, fixed time,
  channel, principal contact, decision maker, SLA), team capacity, knowledge sources,
  gitflow, development flow with an optional design handoff policy, metrics and
  initial risks. Generates five calibrated management agents into
  `.darkside/sith-agents/`. Afterwards runs in four modes: planning (releases,
  story splitting, executable cards, direct Jira sync), execution (pull card →
  Definition of Ready gate → branch → conditional design → build → quality gate →
  PR with AI review → move card → deploy → record cycle data), report (client status,
  metrics with trend against dated snapshots, re-scored risk register, suggested
  adjustments, optional deck) and renegotiate (position, trades, negotiation log).
  Git is proposal-only; Jira is operational. Saves to `.darkside/moff/`.
  Invoke with: `/moff`
...
- 📋 `pm.md` — Product Manager: backlog quality, release cuts, story splitting, Definition of Ready
- ⚠️ `risk-analyst.md` — Risk analyst: probability × impact scoring, warning signals, mitigation effectiveness
- 📈 `delivery-analyst.md` — Delivery analyst: metrics computed from Jira and GitHub, trends, forecast impact
- 🤝 `client-liaison.md` — Client liaison: delivery translated into client terms, surprise detection
- ⚖️ `negotiator.md` — Negotiator: scope trade positions, cost in capacity terms, alternatives that keep the expectation met
...
- `charter.md` — living governance contract: client, success criteria, communication contract, capacity, knowledge sources, gitflow, development flow, metrics, risks, negotiation log
- `risk-register.md` — living risk register, re-scored each round with its history preserved
- `YYYY-MM-DD-<name>-backlog.md` — scope summary + releases + epics and stories + executable cards + Jira sync
- `metrics/YYYY-MM-DD.json` — dated metric snapshot, the basis for trend computation
- `experiments/YYYY-MM-DD-<name>.md` — hypothesis + target metric + result + verdict
- `reports/YYYY-MM-DD-status.md` — executive summary + delivered vs planned + metrics + risks + suggested adjustments + improvement actions + decisions needed + next meeting agenda
